"""
HTAC凝汽器计算程序 - Android直接运行版
可在QPython或Pydroid3中运行
"""
import os
import sys
...
# 工业风格颜色
COLORS = {
    'bg_dark': (0.12, 0.14, 0.16, 1),
    'bg_panel': (0.18, 0.20, 0.23, 1),
    'primary': (0.20, 0.55, 0.85, 1),
    'accent': (1.0, 0.65, 0.0, 1),
    'text': (0.9, 0.9, 0.9, 1),
    'text_secondary': (0.6, 0.6, 0.6, 1),
    'border': (0.3, 0.35, 0.4, 1),
    'input_bg': (0.25, 0.28, 0.32, 1),
...
class InputPanel(BoxLayout):
    ...
    def _create_tube_section(self):
        ...
        self.material = IndustrialSpinner(text='SS TP 304', values=get_material_list())
        grid.add_widget(self.material)
        grid.add_widget(IndustrialLabel(text='流程数:'))
        self.passes = IndustrialSpinner(text='2', values=['1', '2', '4'])
        grid.add_widget(self.passes)
        grid.add_widget(IndustrialLabel(text='冷却水管口数:'))
        self.nozzle_count = IndustrialSpinner(text='2', values=['1', '2'])
    ...
    def _create_mode_section(self):
        ...
        self.mode_temp_rise = ToggleButton(text='输入温升', group='calc_mode', state='down', background_color=COLORS['primary'])
        self.mode_water_flow = ToggleButton(text='输入水量', group='calc_mode', background_color=COLORS['input_bg'])
    ...
    def _create_structure_section(self):
        ...
        self.structure_auto = ToggleButton(text='自动计算', group='structure_mode', state='down', background_color=COLORS['primary'])
        self.structure_manual = ToggleButton(text='手动输入', group='structure_mode', background_color=COLORS['input_bg'])
        self.structure_fixed = ToggleButton(text='固定面积', group='structure_mode', background_color=COLORS['input_bg'])
    ...
    def _on_diameter_change(self, instance, value):
        try:
            ...
            if base_pitch < 25: pitch = 25
            elif base_pitch < 30: pitch = 30
            elif base_pitch <= 32: pitch = 32
            elif base_pitch <= 38: pitch = 38
            else: pitch = base_pitch
...
class ResultPanel(BoxLayout):
    ...
    def _create_result_row(self, label_text, value_text='-'):
        ...
        label = IndustrialLabel(text=label_text, size_hint_x=0.6)
        value = IndustrialLabel(text=value_text, color=COLORS['accent'], size_hint_x=0.4, bold=True)
    ...
    def _create_thermal_section(self):
        ...
        for w in [self.saturation_temp, self.water_enthalpy, self.duty, self.lmtd, self.terminal_diff, 
                  self.u_metric, self.surface_area, self.design_surface]:
    ...
    def _create_structure_section(self):
        ...
        for w in [self.tube_count, self.tube_length, self.tube_sheet_dia, self.length_dia_ratio]:
            self.content.add_widget(w)
    ...
    def _create_hydraulic_section(self):
        ...
        for w in [self.water_flow_kg_s, self.water_flow_m3_h, self.outlet_temp, 
                  self.pressure_drop, self.cw_nozzle, self.cond_nozzle]:
            self.content.add_widget(w)
...
class CondenserCalcApp(App):
    def build(self):
        ...
        title_label = Label(text='◤◤HTAC◢◢ 凝汽器计算程序', font_size='18sp', bold=True, color=COLORS['primary'])
        title_bar.add_widget(title_label)
        root.add_widget(title_bar)
        
        tabs = TabbedPanel(do_default_tab=False, tab_height='40dp', background_color=COLORS['bg_panel'])
        ...
        calc_btn = IndustrialButton(text='执行计算', size_hint_y=None, height='50dp')
    ...
    def on_calculate(self, instance):
        try:
            ...
            errors = []
            if data.steam_pressure is None: errors.append('蒸汽压力')
            if data.steam_mass_flow is None: errors.append('蒸汽流量')
            if data.steam_enthalpy is None: errors.append('蒸汽焓值')
            if data.tube_diameter is None: errors.append('换热管外径')
            if data.tube_wall_thickness is None: errors.append('管壁厚度')
            if data.cooling_water_in_temp is None: errors.append('冷却水进口温度')
            if errors:
    ...
    def show_error(self, message):
        popup = Popup(title='错误', content=Label(text=message, color=COLORS['error']), 
                      size_hint=(0.8, 0.4), background_color=COLORS['bg_panel'])
        popup.open()
    
    def show_success(self, message):
        popup = Popup(title='成功', content=Label(text=message, color=COLORS['success']), 
                      size_hint=(0.6, 0.3), background_color=COLORS['bg_panel'])
        popup.open()
